...
parser.add_argument(
    "--config_path",
    type=str,
    default="./source/evaluation/config_example.json",
...
def prepare_inputs(example, tokenizer, config):
    ...
    # Rename
    qaaseq, query_species, sw_aa_size = (
        example["qseq"],
        example["query_species"],
        config["sw_aa_size"],
    )
    if config["inference_type"] == "mimic":
        ...
        sseq, subject_species = [
            "<mask_" + aa + ">" if aa != "-" else "<gap>" for aa in qaaseq
        ], example["query_species"]

    # Prepare fixed-sized windows
    query_aa_wins = [
        qaaseq[i : i + sw_aa_size]
        for i in range(0, max(1, len(qaaseq) - sw_aa_size + 1))
    ]
    subject_dna_wins = [
        " ".join(sseq[i : i + sw_aa_size])
        for i in range(0, max(1, len(qaaseq) - sw_aa_size + 1))
    ]
    mask_aa_wins = [
        "<"
        + query_species
        + "> "
        + " ".join(["<mask_" + aa + ">" if aa != "-" else "<gap>" for aa in wseq])
        for wseq in query_aa_wins
    ]
    query_aa_wins = [
        "<"
        + query_species
        + "> "
        + get_expr_token(example["expr"])
        + " ".join(["<mask_" + aa + ">" if aa != "-" else "<gap>" for aa in wseq])
        for wseq in query_aa_wins
    ]
    subject_dna_wins = [
        "<" + subject_species + "> " + wseq for wseq in subject_dna_wins
    ]
...
def predict(config, example, mask_restriction_dict, tokenizer, model):
    ...
    res["entropy"] = (
        (-torch.nan_to_num(torch.exp(logits) * logits, nan=0.0).sum(dim=-1))
        .mean()
        .item()
    )
    ...
    if config["calc_stats"] and "query_dna_seq" in example.keys():
        true_vals = tokenizer(example["query_dna_seq"], return_tensors="pt").input_ids[
            :, 1:-1
        ]
        ...
        res["num_of_correct_predicted_codons"] = sum(
