...
class CodonTranslationCollator(object):
    ...
    def __call__(self, features):
        ...
        if self.codon_mask_perc > 0.0:
            mask_aa = self.prepare_mask(
                batch["input_ids"].shape,
                self.codon_mask_perc,
                batch["attention_mask"],
                mask_lens,
            )
            batch["labels"] = torch.where(
                mask_aa[:, 1 : batch["labels"].shape[-1] + 1], batch["labels"], -100
            )  # the +1 is because the expr label is not included in 'labels'.
            batch["input_ids"] = self.mask_input(
                batch["input_ids"], torch.where(mask_aa)
            )
        if self.noise_perc > 0.0:
            mask_noise = self.prepare_mask(
                batch["decoder_input_ids"].shape, self.codon_mask_perc
            )
            batch["decoder_input_ids"] = self.mask_decoder_ids(
                batch["decoder_input_ids"], torch.where(mask_noise)
            )
    ...
    def prepare_mask(
        self, shape, perc, attention_mask=None, per_vector_len_limits=None
    ):
    ...
    def mask_decoder_ids(self, decoder_input_ids, listed_indices):
        ...
        for i, j in zip(listed_indices[0].numpy(), listed_indices[1].numpy()):
            decoder_input_ids[i, j] = self.masking_dict[
                str(decoder_input_ids[i, j].item())
            ]
            k = np.random.choice(
                len(self.restriction_dict[str(decoder_input_ids[i, j].item())])
            )
            decoder_input_ids[i, j] = self.restriction_dict[
                str(decoder_input_ids[i, j].item())
            ][k]
    ...
    def get_batch(self, features):
        ...
        query_win_codons = [
            "<" + f["query_species"] + "> " + " ".join(f["query_dna_seq"][si:ei])
            for (f, si, ei) in zip(features, sinx, einx)
        ]
        subject_win_codons = [
            "<" + f["subject_species"] + "> " + " ".join(f["subject_dna_seq"][si:ei])
            for (f, si, ei) in zip(features, sinx, einx)
        ]
        query_win_input_codons = [
            "<"
            + f["query_species"]
            + "> "
            + self.get_expr_token(f["expr"])
            + " ".join(f["query_dna_seq"][si:ei])
            for (f, si, ei) in zip(features, sinx, einx)
        ]
        ...
        mask_lens = torch.arange(0, batch["input_ids"].shape[-1]).expand(
            batch["input_ids"].shape
        ) < wins_lens.unsqueeze(1)
        batch["labels"] = self.tokenizer(
            query_win_codons, padding="longest", return_tensors="pt"
        )["input_ids"][:, 1:-1]
