...
def get_metrics(tokenizer, training_path=None, special_token_th=31):
    ...
    def compute_metrics_combined(eval_preds):
        ...
        cai_score, cai_bases_score = compute_cai(
            torch.masked_select(labels, mask_codons)
        )
        ...
        return {
            ...
            "acc_bases": torch.mean(acc_bases),
            "acc_codons": torch.mean(acc_codons),
            "cai": cai_score,
            "cai_bases": cai_bases_score,
            "pp": pp,
            "acc_diff": (torch.mean(acc_codons) - cai_score),
        }
