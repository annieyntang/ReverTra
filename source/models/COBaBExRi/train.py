...
parser.add_argument(
    "--config_path",
    type=str,
    default="./source/models/config.json",
...
def load_model(config, checkpoint_flag=False, finetune_flag=False):
    ...
    if (not checkpoint_flag) and finetune_flag:
        ...
        configuration = BartConfig(
            ...
            encoder_layers=model_config["attention_layers"],
            decoder_layers=model_config["attention_layers"],
            encoder_attention_heads=model_config["attention_heads"],
            decoder_attention_heads=model_config["attention_heads"],
            encoder_ffn_dim=model_config["ffn_dim"],
            decoder_ffn_dim=model_config["ffn_dim"],
...
def init_trainer(
    training_args, tokenizer, dataset, masking_dict, restriction_dict, config
):
    metrics = get_metrics(
        tokenizer,
        config["cai_refference_path"],
        special_token_th=config["special_token_th"],
    )
    training_collator = CodonTranslationCollator(
        tokenizer,
        masking_dict,
        restriction_dict,
        codon_mask_perc=config["train_config"][
            "mask_perc"
        ],  # Should be zero - 100% mask on single species (in the dataset), and 0% on translation (homologs).
...
if __name__ == "__main__":
    ...
    dataset = load_dataset(
        config["dataset_path"],
        single_species_flag=config["dataset_single_species_flag"],
        finetune_flag=config["finetune_flag"],
    )
    tokenizer, masking_dict, restriction_dict = load_tokenizer(config["tokenizer_path"])
    model = load_model(
        config,
        checkpoint_flag=config["checkpoint_flag"],
        finetune_flag=config["finetune_flag"],
    )
    train_args = init_training_args(
        config["train_config"],
        os.path.join(config["model_outpath"], config["model_name"]),
        len(dataset["train"]),
    )
    trainer = init_trainer(
        train_args, tokenizer, dataset, masking_dict, restriction_dict, config
    )
    trainer.train()
