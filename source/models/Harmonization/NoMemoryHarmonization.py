...
class Harmonizer(object):
    def __init__(
        self,
        dataset_path=".data/datasets/processed_data_SCPECBS3",
        ...
        self.cai_species_index = self.init_cai_translation_dict(
            dataset_path, species_list, cai_type
        )
    ...
    def init_CAI(self, path):
        ...
        for aa in AAs:
            ...
            codons_for_aa_codons = [
                (c, cai_data["CAI_probs"][c]) for c in self.codons if translate(c) == aa
            ]
    ...
    def init_cai_translation_dict(self, dataset_path, species_list, cai_type):
        ...
        for s in species_list:
            d[s] = self.init_CAI(
                os.path.join(os.path.join(dataset_path, s), s + cai_type)
            )
        return d

    def calc_harmonic_acc(
        self,
        origin_seq,
        dest_seq,
        origin_species="S_pombe",
        destination_species="S_cerevisiae",
    ):
        ...
        accuracy = sum([x == y for (x, y) in zip(dest_seq, dest_pred_seq)]) / len(
            dest_seq
        )
        """
        print(sc_aaidentity)
