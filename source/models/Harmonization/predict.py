...
def predict(example, mask_restriction_dict, tokenizer, model):
    ...
    accuracy, harmonized_seq = model.calc_harmonic_acc(
        subject_seq, query_seq, example["subject_species"], example["query_species"]
    )

    res = dict()
    res["num_of_correct_predicted_codons"] = sum(
        [1 for x, y in zip(true_vals, harmonized_seq.split(" ")) if (x == y)]
    )
