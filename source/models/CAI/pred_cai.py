...
def predict(example, mask_restriction_dict, tokenizer, model):
    ...
    res["num_of_correct_predicted_codons"] = sum(
        [1 for x, y in zip(true_vals, cai_seq) if (x == y)]
    )
    ...
    res["cross_entropy_loss"] = ce(
        cai_logits.cuda(), torch.tensor(true_vals).cuda()
    ).item()
    res["entropy"] = (
        (-torch.nan_to_num(torch.exp(cai_logits) * cai_logits, nan=0.0).sum(dim=-1))
        .mean()
        .item()
    )
