...
class CAI(object):
    ...
    def get_cai_vectorized_representations(self, tokenizer):
        cai_vec_rep = {
            aa: torch.ones(1, len(tokenizer)) * float("inf") * (-1)
            for aa in self.unique_aa
        }
        for aa in self.unique_aa:
            for ci in self.codons_for_aa[aa]:
                cai_vec_rep[aa][0, tokenizer.convert_tokens_to_ids(self.codons[ci])] = (
                    self.CAI_probs[self.codons[ci]]
                )
        return cai_vec_rep
    ...
    def init_AA_codons(self):
        ...
        for aa in self.unique_aa:
            codons_for_aa[aa] = [
                x for x, i in enumerate(self.codons_translated_to_aa) if i == aa
            ]
    ...
    def normalize_codons_count(self, codons_count):
        ...
        for a in self.unique_aa:
            sum_AA = sum(
                [codons_count[self.codons[ci]] for ci in self.codons_for_aa[a]]
            )
    ...
    def calc_cai_acc(self, dna_seq):
        ...
        acc = sum(x == y for x, y in zip(true_codon_s1, cai_codon_s1)) / len(
            true_codon_s1
        )
