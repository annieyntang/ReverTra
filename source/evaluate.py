...
parser.add_argument("--out_filename", type=str, default="res_fixed.csv", help="")
...
def single_species_mask(example):
    return {
        "subject_dna_seq": [
            "<mask_" + aa + ">" if aa != "-" else "<gap>" for aa in example["qseq"]
        ]
    }
...
def load_dataset(config):
    ...
    if config["mask_all"]:  # if single species leave only AA seq of query as input.
        ...
        print("only full seq")
        codon_dataset["test"] = codon_dataset["test"].map(
    ...
    if config["eval_type"] == "cai":
        codon_dataset = codon_dataset.filter(
            lambda example: example["query_species"] == config["cai_query_species"]
        )
        print(codon_dataset)
...
if __name__ == "__main__":
    ...
    tokenizer, masking_dict, restriction_dict = module.load_tokenizer(
    ...
    for i in tqdm(range(num_entries_in_ds)):
        ...
        if config["eval_type"] == "cai":
            res.update(
                CAI.predict_cai(
                    codon_dataset["test"][i], restriction_dict, tokenizer, model
                )
            )
        elif config["eval_type"] == "hrm":
            res.update(
                Harmonization.predict_harmonize(
                    codon_dataset["test"][i], None, None, model
                )
            )
        else:  # args.eval_type == 'model':
            res.update(
                module.predict_model(
                    config, codon_dataset["test"][i], restriction_dict, tokenizer, model
                )
            )
