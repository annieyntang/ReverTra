...
parser = argparse.ArgumentParser(
    description="get subset of sequences with high expression level for CAI calculation."
)
parser.add_argument("--species", type=str, default="S_cerevisiae", help="")
parser.add_argument("--data_path", type=str, required=True, help="")
parser.add_argument("--expr_path", type=str, required=True, help="")
parser.add_argument("--expr_th_perc", type=float, default="0.05", help="")

args = parser.parse_args()

if __name__ == "__main__":
    training_file_path = os.path.join(
        os.path.join(args.data_path, args.species), args.species + ".0.nt.fasta"
    )
    ...
    subset_output = os.path.join(
        os.path.join(args.data_path, args.species),
        args.species + ".subset.e" + str(args.expr_th_perc) + ".0.nt.fasta",
    )
