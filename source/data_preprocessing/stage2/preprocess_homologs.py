...
parser.add_argument(
    "--mode", type=str, default="homologs", help="Script mode: homologs or augmented."
)
parser.add_argument(
    "--cut_perc",
    type=float,
...
def generate_dictionaries(data_path, partition_id):
    protO1 = os.path.join(
        data_path, args.species1 + "." + str(partition_id) + ".aa.fasta"
    )
    protO2 = os.path.join(
        data_path, args.species2 + "." + str(partition_id) + ".aa.fasta"
    )
    dnaO1 = os.path.join(
        data_path, args.species1 + "." + str(partition_id) + ".nt.fasta"
    )
    dnaO2 = os.path.join(
    ...
    seq_dict2 = {
        rec.id.replace("trans_", ""): rec.seq for rec in SeqIO.parse(protO2, "fasta")
    }
...
def preprocess_homologs(data_path, partition_id, sw_size, sw_jump):
    ...
    aligned_sequences["qseqid"] = [
        s.replace("trans_", "") for s in aligned_sequences["qseqid"]
    ]
    aligned_sequences["sseqid"] = [
        s.replace("trans_", "") for s in aligned_sequences["sseqid"]
    ]

    print(aligned_sequences.head())
    homologs_outfile = os.path.join(
        args.data_path,
        args.species1
        + "_"
        + args.species2
    ...
    for i in range(len(aligned_sequences)):
        ...
        query_dna, subject_dna = get_codons_for_aa(
            seq_dict_dna[name1], s1, seq_dict_dna[name2], s2
        )
...
if __name__ == "__main__":
    if args.mode == "homologs":
        preprocess_homologs(
            args.data_path, args.partition_id, args.win_size, args.sw_jump
        )
    else:
