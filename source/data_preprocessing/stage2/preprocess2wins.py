...
parser.add_argument(
    "--data_path",
    type=str,
    default="/home/tomer/CodOpTRM/data/homologs_data/processed_test",
    help="location of the partition fasta files.",
)
parser.add_argument(
    "--log_path",
    type=str,
    default="/home/tomer/CodOpTRM/data/homologs_data/logs",
...
def alignment2wins(query_seq_dna, query_seq_aa, subject_seq_dna, subject_seq_aa):
    ...
    for i in range(0, nwins):
        si = i * ws
        win_dict["query_dna_seq"].append(
            " ".join(query_seq_dna[i * ws : min((i + 1) * ws, len(query_seq_dna))])
        )
        win_dict["qseq"].append(
            query_seq_aa[int(i * ws) : min(int((i + 1) * ws), len(query_seq_aa))]
        )
        win_dict["subject_dna_seq"].append(
            " ".join(subject_seq_dna[i * ws : min((i + 1) * ws, len(subject_seq_dna))])
        )
        win_dict["sseq"].append(
            subject_seq_aa[int(i * ws) : min(int((i + 1) * ws), len(subject_seq_aa))]
        )
    return win_dict, nwins


def preprocess_homologs(data_path):
    homologs_pairs = pd.read_csv(
        os.path.join(
            args.data_path,
            args.species1
            + "_"
    ...
    for i in range(len(homologs_pairs)):
        ...
        aligned_wins, nwins = alignment2wins(
            idata["query_dna_seq"].split(" "),
            idata["qseq"],
            idata["subject_dna_seq"].split(" "),
            idata["sseq"],
        )
